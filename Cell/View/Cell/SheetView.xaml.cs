            ...
            if (ViewUtilities.TryGetSendersDataContext<CellViewModel>(sender, out var cell))
            {
                if (e.ChangedButton == MouseButton.Left)
                {
                    ...
                    if (wasSelected)
                    {
                        ...
                        {
                            foreach (var rowCell in Data.Cells.Instance.GetCellModelsForSheet(SheetViewModel.SheetName).Where(x => x.Row == cell.Model.Row))
                            {
                        ...
                        {
                            foreach (var columnCell in Data.Cells.Instance.GetCellModelsForSheet(SheetViewModel.SheetName).Where(x => x.Column == cell.Model.Column))
                            {
                    ...
                    if (e.LeftButton == MouseButtonState.Pressed && _selectingCells)
                    {
                        ...
                        else
                        {
                            ...
                            for (var row = startRow; row <= endRow; row++)
                            {
                                ...
                                {
                                    var cellToSelect = Data.Cells.Instance.GetCell(SheetViewModel.SheetName, row, column);
                                    if (CanSelectCell(cellToSelect)) SheetViewModel.SelectCell(cellToSelect!);
                                }
                            }
                            var topLeftCell = Data.Cells.Instance.GetCell(SheetViewModel.SheetName, startRow, startColumn);
