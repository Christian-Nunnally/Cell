...
namespace Cell.ViewModel
{
    public class RowCellViewModel : SpecialCellViewModel
    {
        ...
        {
            if (_sheetViewModel.RowCellViewModels.Count() == 1) return;
        ...
        private void AddRowAt(int rowToInsertAt)
        {
            IncrementRowOfAllAtOrBelow(rowToInsertAt);
            InsertRowAtIndex(rowToInsertAt);
            _sheetViewModel.UpdateLayout();
        }
        ...
        private void InsertRowAtIndex(int index)
        {
            ...
            var columnIndexs = _sheetViewModel.ColumnCellViewModels.Select(x => x.Column).ToList();
        ...
        private static void IncrementRowOfAllAtOrBelow(int row, int amount = 1)
        {
            var cells = GetAllCellsAtOrBelow(row);
            foreach (var cell in cells) cell.Row += amount;
        }

        private static IEnumerable<CellModel> GetAllCellsAtOrBelow(int row) => Cells.AllCells.Where(x => x.Row >= row);
